...
test("blog publishing is reserved for the platform owner at launch", () => {
  ...
  for (const role of [
    ...
    assert.equal(adminRoleHasPermission(role, "blog:write"), false);
...
test("viewer and analyst roles are read-only", () => {
  ...
  assert.equal(
    adminRoleHasPermission("admin_viewer", "invitations:manage"),
    false,
  );
...
test("permission reverse lookup exposes eligible admin roles", () => {
  ...
  assert.ok(
    getAdminRolesForPermission("disputes:manage").includes("support_officer"),
  );
});

test("warehouse managers can supervise schedules and runs but remain warehouse scoped", () => {
  assert.equal(
    adminRoleHasPermission("warehouse_manager", "marketSchedules:read"),
    true,
  );
  assert.equal(
    adminRoleHasPermission("warehouse_manager", "marketSchedules:manage"),
    true,
  );
  assert.equal(
    adminRoleHasPermission("warehouse_manager", "marketRuns:read"),
    true,
  );
  assert.equal(
    adminRoleHasPermission("warehouse_manager", "marketRuns:manage"),
    true,
  );
  assert.equal(
    adminRoleHasPermission("warehouse_manager", "adminAccess:manage"),
    false,
  );
});
...
test("warehouse scope grants never match another warehouse", () => {
  ...
  assert.equal(
    adminScopeMatchesTarget(managerGrant, { warehouseId: "warehouse-a" }),
    true,
  );
  assert.equal(
    adminScopeMatchesTarget(managerGrant, { warehouseId: "warehouse-b" }),
    false,
  );
  assert.equal(
    adminScopeMatchesTarget(managerGrant, { destinationMarket: "Tarkwa" }),
    false,
  );
});
  ...
  for (const scopeType of [
    ...
    assert.equal(
      adminScopeMatchesTarget(
        ...
        { region: "ashanti", district: "kumasi", destinationMarket: "makola" },
      ),
      false,
    );
  }
