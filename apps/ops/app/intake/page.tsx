...
export default function IntakePage() {
  ...
  const photoPreviewUrl = useMemo(
    () => (photoFile === null ? undefined : URL.createObjectURL(photoFile)),
    [photoFile],
  );
    ...
    try {
      ...
      setPhotoUploadError(
        error instanceof Error
          ? error.message
          : "Could not upload the produce photo.",
      );
    } finally {
  ...
  const varietiesByCrop: Record<string, string[]> = {
    Maize: [
      "Obatanpa Quality Protein Maize",
  ...
  useEffect(() => {
    if (draftIntake) {
      ...
      if (draftIntake.conditionNotes)
        setConditionNotes(draftIntake.conditionNotes);
      ...
      if (draftIntake.shelfLifeDays)
        setShelfLifeDays(draftIntake.shelfLifeDays);
      if (draftIntake.storageRate) setStorageRate(draftIntake.storageRate);
      if (draftIntake.storageRateRuleId)
        setStorageRateRuleId(draftIntake.storageRateRuleId);
  ...
  const selectedStorageRateRule = storageRateRules.find(
    (rule) => rule.id === storageRateRuleId,
  );
  ...
  const handleSubmit = () => {
    ...
    void addIntake(intakePayload)
      .then((batch) => {
        showToast("Produce intake recorded and receipt created.");
      ...
      .catch((error: unknown) => {
        setSubmitError(
          error instanceof Error
            ? error.message
            : "Could not create intake receipt.",
        );
      })
        ...
        <div className="step-container">
                ...
                >
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: "13px", color: "var(--gray-600)" }}>
                      {selectedFarmer.phoneNumber} · {selectedFarmer.community}
                    </div>
                  </div>
                </div>
                <button
                  ...
                  {filteredFarmers.map((farmer) => (
                    ...
                    >
                      <div>
                        ...
                        >
                          {farmer.phoneNumber} · {farmer.community}
                        </div>
                      </div>
                      <div className="code-chip" style={{ fontSize: "11px" }}>
                        {farmer.farmerCode}
                      </div>
                    </div>
                  ))}
          ...
          >
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="warehouseSelect">
                Select Facility
              </label>
              <select
                id="warehouseSelect"
              ...
              >
                {assignedWarehouses.map((warehouse) => (
                  <option key={warehouse.id} value={warehouse.id}>
                    {warehouse.name}
                    {warehouse.id === activeWarehouse.id ? " (current)" : ""}
                  </option>
                ))}
              </select>
            </div>
        ...
        <div className="step-container">
          ...
          {cropType && (
            ...
            >
              ...
              <div className="form-group">
                <label className="form-label" htmlFor="varietySelect">
                  Crop Variety
                </label>
                <select
                  id="varietySelect"
              ...
              <div className="form-group">
                <label className="form-label" htmlFor="conditionText">
      ...
      {step === 3 && (
        <div className="step-container">
          ...
          >
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="storageRate">
                Storage Fee Rate (GHS)
              </label>
              {storageRateRules.length > 0 && (
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const rule = storageRateRules.find(
                      (item) => item.id === nextRuleId,
                    );
                    if (rule) {
                ...
                >
                  ...
                  {storageRateRules
                    .filter(
                      (rule) =>
                        (!rule.cropType || rule.cropType === cropType) &&
                        rule.unit === unit &&
                        (!rule.grade || rule.grade === grade),
                    )
                    .map((rule) => (
                      <option key={rule.id} value={rule.id}>
                        {rule.cropType || "Any crop"} / {rule.unit} /{" "}
                        {rule.grade || "any grade"} - GHS{" "}
                        {rule.ratePerUnitPerDay.toFixed(2)}
                      </option>
                    ))}
                </select>
              )}
              <div style={{ position: "relative" }}>
                ...
                <input
                  ...
                  value={storageRate}
                  onChange={(e) =>
                    setStorageRate(Math.max(0, parseFloat(e.target.value) || 0))
                  }
                />
              </div>
              <div
              ...
              >
                ...
                {selectedStorageRateRule ? " from active storage rule." : "."}
              </div>
            </div>
          </div>
          ...
          >
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="askingPrice">
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="minimumPrice">
                Minimum Price per unit (Optional)
              </label>
              <div style={{ position: "relative" }}>
                <span
                  style={{
          ...
          >
            ...
            >
              <div>
                <h2 className="detail-section-title">
                  <Camera size={18} /> 4. Produce photo
                </h2>
                <p
                  style={{
                    margin: "6px 0 0",
                ...
                >
                  Optional. Upload it now so it is ready before you review and
                  confirm the intake.
                </p>
              </div>
            ...
            {photoPreviewUrl === undefined ? (
              ...
              >
                ...
                <button
                  ...
                  onClick={() => void handlePhotoSelection(null)}
                  style={{
                    position: "absolute",
                    right: "10px",
                    top: "10px",
            ...
            {isUploadingPhoto ? (
              <div className="offline-banner" style={{ margin: 0 }}>
                <Info size={16} />
                <span>Uploading photo before review…</span>
              </div>
            ) : null}
            {photoAssetId !== undefined ? (
              ...
              >
                Photo uploaded and ready for this intake.
              </div>
            ) : null}
            {photoUploadError ? (
              <div
              ...
              >
                ...
                <span>
                  {photoUploadError} Choose the photo again to retry, or remove
                  it to continue without one.
                </span>
              </div>
            ) : null}
          </div>
          ...
          <button
            ...
            disabled={
              isUploadingPhoto ||
              (photoFile !== null && photoAssetId === undefined)
            }
            onClick={() => saveDraft(4)}
          >
        ...
        <div className="step-container">
            ...
            <div className="receipt-body">
              <div className="receipt-fact-grid">
                ...
                <div className="fact-item">
                  ...
                  <span className="fact-value-large">
                    {quantity}{" "}
                ...
                <div className="fact-item">
                  ...
                  <span className="fact-value">
                    {askingPrice
                      ? `GHS ${parseFloat(askingPrice).toFixed(2)}`
                      : "Not specified"}
                  </span>
                </div>
                <div className="fact-item">
                  ...
                  <span className="fact-value">
                    {minimumPrice
                      ? `GHS ${parseFloat(minimumPrice).toFixed(2)}`
                      : "Not specified"}
                  </span>
                </div>
              </div>
              ...
              {conditionNotes && (
                ...
                >
                  <div className="fact-label" style={{ marginBottom: "4px" }}>
                    Condition Notes
                  </div>
                  <div
                    style={{
              ...
              <div className="receipt-callout">
                ...
                >
                  ...
                  />
                  <span>
                    Confirm these collection details with the farmer. Physical
                    copy or SMS notification will be generated upon commitment.
                  </span>
                </div>
              </div>
            </div>
          ...
          {isOffline && (
            <div className="offline-banner" style={{ margin: "0" }}>
              <Info size={16} />
              <span>
                Offline Mode active: This receipt will be cached locally and
