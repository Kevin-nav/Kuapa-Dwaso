...
`/auth` provides the Firebase email/password entry path for admins and warehouse managers. Configure `NEXT_PUBLIC_FIREBASE_*`, `NEXT_PUBLIC_CONVEX_URL`, and `NEXT_PUBLIC_API_URL` in the repo root `.env.local` for local development; app-local env files are optional overrides only.
