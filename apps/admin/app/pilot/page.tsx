...
import { AlertTriangle, CheckCircle2, CircleDollarSign, ShieldCheck, Sprout, UserPlus } from "lucide-react";
...
const capabilities = ["pilot:read", "requests:review", "supply:manage", "offers:manage", "quality:record", "fulfilment:manage", "custody:record", "issues:manage"] as const;
...
export default function PilotAdminPage() {
  ...
  const access = useQuery(api.adminAccess.getEffectiveAccess, actorUserId === undefined ? "skip" : { actorUserId, adminUserId: actorUserId });
  ...
  const issues = useQuery(api.pilotIssues.listForProgramme, selected === undefined ? "skip" : { programmeId: selected.id }) as Array<{ _id: string; requestId: string; status: string; summary: string; nextStep: string; deadlineAt?: number }> | undefined;
  const assignments = useQuery(api.pilotAssignments.list, selected === undefined ? "skip" : { programmeId: selected.id, status: "active", limit: 50 }) as { page: Array<{ assignmentId: Id<"pilotAssignments">; userId: Id<"users">; capabilities: string[]; version: number }> } | undefined;
  const candidates = useQuery(api.pilotAssignments.listCandidates, selected === undefined ? "skip" : { programmeId: selected.id }) as Array<{ userId: Id<"users">; name: string; identityKind: string; warehouseCount: number }> | undefined;
  const finance = useQuery(api.pilotFinance.getProgrammeSummary, selected === undefined || !canReadFinance ? "skip" : { programmeId: selected.id }) as FinanceSummary | undefined;
  ...
  const overdueIssues = openIssues.filter((issue) => issue.deadlineAt !== undefined && issue.deadlineAt < now);
  const atRiskCollections = requestRows.filter((request) => request.deliveryWindowEndAt < now && !["delivered", "closed", "cancelled"].includes(request.status));
  const outstanding = (entries?.page ?? []).filter((entry) => entry.postingKind === "obligation").reduce((sum, entry) => sum + (entry.obligation?.outstandingPesewas ?? 0), 0);
    ...
    <main className="pilot-admin">
      ...
      {selected === undefined ? <CreateProgramme canManage={canManageProgrammes} demoPresentation={demoPresentation} /> : <>
        ...
        <section className="pilot-admin__metrics" aria-label="Priority risks">
          ...
          <Metric label="Collection risk" value={atRiskCollections.length} detail="Past delivery window" danger={atRiskCollections.length > 0} />
          <Metric label="Unpaid obligations" value={money(outstanding)} detail="Actual outstanding ledger" danger={outstanding > 0} />
        </section>

        <div className="pilot-admin__grid">
          <section className="pilot-admin__panel pilot-admin__panel--wide">
            <div className="pilot-admin__panel-head"><div><span className="pilot-admin__eyebrow">Demand and exceptions</span><h2>Work that needs attention</h2></div></div>
            <div className="pilot-admin__request-list">
              {requestRows.map((request) => <Link key={request.requestId} href={`/pilot/requests/${request.requestId}`} className="pilot-admin__request"><div><strong>{kg(request.confirmedGrams ?? request.requestedGrams)} maize</strong><span>{request.commercialMode.replaceAll("_", " ")} · {request.status.replaceAll("_", " ")}</span></div><span>Statement →</span></Link>)}
              {requestRows.length === 0 ? <p className="pilot-admin__muted">No requests are recorded for this programme.</p> : null}
            </div>
            {openIssues.length > 0 ? <div className="pilot-admin__issues">{openIssues.slice(0, 5).map((issue) => <article key={issue._id}><AlertTriangle size={18} /><div><strong>{issue.summary}</strong><p>{issue.nextStep}</p><small>{issue.deadlineAt === undefined ? "No deadline" : new Date(issue.deadlineAt).toLocaleString()}</small></div></article>)}</div> : null}
          </section>

          <ProgrammeConfiguration programme={selected} canManage={canManageProgrammes} />
...
function ProgrammeConfiguration({ programme, canManage }: { programme: Programme; canManage: boolean }) {
  ...
  const [maizeType, setMaizeType] = useState(current?.qualityPolicy?.maizeType ?? "Yellow maize");
  const [moisture, setMoisture] = useState(String((current?.qualityPolicy?.moistureMaximumPermille ?? 135) / 10));
  const [fee, setFee] = useState(String((current?.chargeTerms?.[0]?.rate.numerator ?? 25) / (current?.chargeTerms?.[0]?.rate.scale ?? 1) / 100));
  const [paymentDays, setPaymentDays] = useState(String(current?.paymentTerms?.[0]?.offsetCalendarDays ?? 2));
  const [limit, setLimit] = useState(String((current?.purchaseLimitPesewas ?? 500_000) / 100));
  const [reference, setReference] = useState(current?.approvalReferences.join(", ") ?? (programme.datasetProvenance === "sample_only" ? "DEMO-ASSUMPTION-01" : ""));
  ...
  async function save(configurationStatus: "draft" | "approved") {
    ...
    try {
      await configure({
        ...
        configurationStatus,
        qualityPolicy: { maizeType: maizeType.trim(), moistureMaximumPermille: Math.round(Number(moisture) * 10), contaminationCheckRequired: true, additionalCriteria: [{ code: "VISIBLE_MOULD", label: "No visible mould", required: true }], policyProvenance: programme.datasetProvenance },
        chargeTerms: [{ code: "COORDINATION_FEE", label: "Coordination fee", payer: "buyer", calculation: "per_kg", rate: { numerator: Math.round(Number(fee) * 100), scale: 1, unit: "per_kg" } }],
        paymentTerms: [{ trigger: "buyer_acceptance", offsetCalendarDays: Number(paymentDays), timezone: "Africa/Accra" }],
        purchaseLimitPesewas: Math.round(Number(limit) * 100),
        taxTerms: [{ code: "TAX_REVIEW", label: "Tax treatment", detail: "Apply only documented taxes for the transaction; no tax is inferred by the demo." }],
        approvalReferences: reference.split(",").map((item) => item.trim()).filter(Boolean),
        expectedVersion: programme.version,
        idempotencyKey: crypto.randomUUID(),
      });
      setMessage(configurationStatus === "approved" ? "Configuration approved." : "Draft saved.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Configuration was not saved."); } finally { setBusy(false); }
  }

  async function changeStatus(status: Programme["status"]) {
    setBusy(true); setMessage(undefined);
    try { await setStatus({ programmeId: programme.id, status, expectedVersion: programme.version, reason: status === "active" ? "Approved for pilot operations" : "Administrative programme state change", idempotencyKey: crypto.randomUUID() }); setMessage(`Programme changed to ${status}.`); }
    catch (error) { setMessage(error instanceof Error ? error.message : "Programme status was not changed."); } finally { setBusy(false); }
  }
...
function AssignmentManager({ programme, assignments, candidates, canManage }: { programme: Programme; assignments: Array<{ assignmentId: Id<"pilotAssignments">; userId: Id<"users">; capabilities: string[]; version: number }>; candidates: Array<{ userId: Id<"users">; name: string; identityKind: string; warehouseCount: number }>; canManage: boolean }) {
  const grant = useMutation(api.pilotAssignments.grant); const revoke = useMutation(api.pilotAssignments.revoke);
...
function FinancePanel({ summary, queue, canRead, canManage, programme }: { summary: FinanceSummary | undefined; queue: ApprovalRow[]; canRead: boolean; canManage: boolean; programme?: Programme }) {
  ...
  return <section className="pilot-admin__panel pilot-admin__panel--wide"><div className="pilot-admin__panel-head"><div><span className="pilot-admin__eyebrow">Finance-only actions</span><h2>Purchasing exposure and actuals</h2></div><CircleDollarSign size={24} /></div>
    <div className="pilot-admin__finance-grid"><FinanceMetric label="Gross produce value" value={actuals?.buyerProducePesewas} /><FinanceMetric label="Coordination fees" value={actuals?.coordinationRevenuePesewas} /><FinanceMetric label="Farmer liabilities" value={actuals?.farmerPayablesPesewas} /><FinanceMetric label="Operating costs" value={actuals?.operatingCostPesewas} /><FinanceMetric label="Cash received" value={actuals?.buyerReceiptsPesewas} /><FinanceMetric label="Actual contribution" value={actuals?.contribution.status === "complete" ? actuals.contribution.amountPesewas : undefined} incomplete={actuals?.contribution.status === "incomplete"} /></div>
    <p className="pilot-admin__muted">Gross produce value is not revenue. Contribution remains incomplete until required actual costs and corrections are recorded.</p>
    <h3>Purchase approvals</h3>{budget === undefined ? <><p className="pilot-admin__warning">No active purchasing budget. A finance manager must create approved capacity with private funding evidence.</p>{canManage && programme !== undefined ? <div className="pilot-admin__form pilot-admin__form--row"><label>Funding source<input value={budgetForm.label} onChange={(event) => setBudgetForm((current) => ({ ...current, label: event.target.value }))} /></label><label>Approval reference<input value={budgetForm.reference} onChange={(event) => setBudgetForm((current) => ({ ...current, reference: event.target.value }))} /></label><label>Capacity (GHS)<input type="number" min="0.01" value={budgetForm.capacity} onChange={(event) => setBudgetForm((current) => ({ ...current, capacity: event.target.value }))} /></label><label>Private funding evidence<input type="file" accept="image/*,.pdf" onChange={(event) => setBudgetEvidence(event.target.files?.[0])} /></label><button className="primary" disabled={firebaseUser === null || budgetEvidence === undefined} onClick={() => firebaseUser === null || budgetEvidence === undefined ? undefined : void uploadFinancialEvidence({ user: firebaseUser, file: budgetEvidence, programmeId: programme.id, relatedEntityType: "pilotProgrammes", relatedEntityId: programme.id }).then((assetId) => createBudget({ programmeId: programme.id, fundingSourceLabel: budgetForm.label, fundingSourceReference: budgetForm.reference, fundingEvidenceUploadAssetIds: [assetId as Id<"uploadAssets">], approvedCapacityPesewas: Math.round(Number(budgetForm.capacity) * 100), datasetProvenance: programme.datasetProvenance, idempotencyKey: crypto.randomUUID() })).then(() => setMessage("Purchasing budget created from the documented funding source.")).catch((error: unknown) => setMessage(error instanceof Error ? error.message : "Budget was not created."))}>Create purchasing budget</button></div> : null}</> : <p className="pilot-admin__budget">Available capacity <strong>{money(budget.availablePesewas)}</strong> · {budget.fundingSourceLabel}</p>}
    <div className="pilot-admin__queue">{queue.map((row) => <div key={row.farmerOfferRevisionId}><div><strong>{row.farmerName} · {kg(row.offeredGrams)}</strong><span>{money(row.expectedNetPesewas)} farmer proceeds · {row.approvalStatus.replaceAll("_", " ")}</span></div>{row.approvalStatus === "awaiting_approval" ? <div className="pilot-admin__approve"><label>Known costs (GHS)<input type="number" min="0" value={costByRevision[row.farmerOfferRevisionId] ?? "0"} onChange={(event) => setCostByRevision((current) => ({ ...current, [row.farmerOfferRevisionId]: event.target.value }))} /></label><button disabled={!canManage || budget === undefined} onClick={() => budget === undefined ? undefined : void reserve({ budgetId: budget.budgetId, requestId: row.requestId, buyerAgreementRevisionId: row.buyerAgreementRevisionId, farmerOfferRevisionId: row.farmerOfferRevisionId, produceAmountPesewas: row.expectedNetPesewas, knownCostAmountPesewas: Math.round(Number(costByRevision[row.farmerOfferRevisionId] ?? 0) * 100), expiresAt: Math.max(Date.now() + 3_600_000, row.expiresAt), expectedBudgetVersion: budget.version, idempotencyKey: crypto.randomUUID() }).then(() => setMessage("Purchase capacity reserved.")).catch((error: unknown) => setMessage(error instanceof Error ? error.message : "Purchase approval failed."))}>Approve funding</button></div> : <StatusPill>approved</StatusPill>}</div>)}</div>
...
type ApprovalRow = { requestId: Id<"pilotBuyerRequests">; buyerAgreementRevisionId: Id<"pilotBuyerAgreementRevisions">; farmerOfferRevisionId: Id<"pilotFarmerOfferRevisions">; farmerName: string; offeredGrams: number; expectedNetPesewas: number; expiresAt: number; approvalStatus: "approved" | "awaiting_approval" };
type FinanceSummary = { budgets: Array<{ budgetId: Id<"pilotPurchasingBudgets">; fundingSourceLabel: string; availablePesewas: number; status: string; version: number }>; actuals: { buyerProducePesewas: number; farmerPayablesPesewas: number; coordinationRevenuePesewas: number; operatingCostPesewas: number; buyerReceiptsPesewas: number; contribution: { status: "complete"; amountPesewas: number } | { status: "incomplete" } } };
function Metric({ label, value, detail, danger = false }: { label: string; value: string | number; detail: string; danger?: boolean }) { return <article className={`pilot-admin__metric${danger ? " pilot-admin__metric--danger" : ""}`}><span>{label}</span><strong>{value}</strong><small>{detail}</small></article>; }
function FinanceMetric({ label, value, incomplete = false }: { label: string; value: number | undefined; incomplete?: boolean }) { return <div><span>{label}</span><strong>{incomplete ? "Incomplete" : money(value ?? 0)}</strong></div>; }
function StatusPill({ children }: { children: string }) { return <span className="pilot-admin__pill">{children.replaceAll("_", " ")}</span>; }
function StateCard({ title, detail }: { title: string; detail: string }) { return <main className="pilot-admin"><section className="pilot-admin__panel"><h1>{title}</h1><p>{detail}</p></section></main>; }
function money(pesewas: number) { return `GHS ${(pesewas / 100).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`; }
function kg(grams: number) { return `${(grams / 1000).toLocaleString()} kg`; }
