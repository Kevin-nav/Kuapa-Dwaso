...
export default function MarketServicesPage() {
  ...
  return (
    <div style={{ display: "grid", gap: 24 }}>
      ...
      <section style={panelStyle}><h2 style={sectionTitle}>Create dated delivery run</h2><form onSubmit={(event) => { event.preventDefault(); if (runScheduleId !== undefined && runDate && actorUserId) void createRun({ actorUserId, scheduleId: runScheduleId, deliveryDate: runDate }).then((id) => { setSelectedRunId(id); setMessage("Draft run created."); }).catch((error: unknown) => setMessage(error instanceof Error ? error.message : "Run could not be created.")); }} style={{ display: "flex", gap: 10, flexWrap: "wrap" }}><select required value={runScheduleId ?? ""} onChange={(e) => setRunScheduleId(e.target.value as Id<"marketServiceSchedules">)} style={inputStyle}><option value="">Choose active service</option>{activeSchedules.map((schedule) => <option key={schedule._id} value={schedule._id}>{schedule.warehouseName} → {schedule.destinationName}</option>)}</select><input required type="date" value={runDate} onChange={(e) => setRunDate(e.target.value)} style={inputStyle} /><button style={primaryButton}>Create run safely</button></form></section>

      <section style={panelStyle}><h2 style={sectionTitle}>Upcoming delivery runs</h2><div style={cardsGridStyle}>{(runs ?? []).sort((a, b) => a.deliveryDateAt - b.deliveryDateAt).map((run) => <button key={run._id} onClick={() => setSelectedRunId(run._id)} style={{ ...cardStyle, textAlign: "left", cursor: "pointer", borderColor: selectedRunId === run._id ? palette.field : gray[100] }}><div style={rowStyle}><strong>{run.destinationName} · {formatDate(run.deliveryDateAt, run.timezone)}</strong><StatusBadge status={run.status} /></div><p style={smallText}>Orders close {formatDateTime(run.orderCutoffAt, run.timezone)} · expected {formatTime(run.expectedArrivalStartAt, run.timezone)}–{formatTime(run.expectedArrivalEndAt, run.timezone)}</p><span style={{ ...smallText, fontWeight: 800 }}>{run.buyerOrderIds.length} buyer order(s) · open readiness view</span></button>)}</div></section>

      {readiness && <section style={panelStyle}><div style={rowStyle}><div><h2 style={sectionTitle}>Run readiness: {readiness.destinationName}</h2><p style={smallText}>Cutoff {formatDateTime(readiness.orderCutoffAt, readiness.timezone)} · arrival {formatDateTime(readiness.expectedArrivalStartAt, readiness.timezone)}–{formatTime(readiness.expectedArrivalEndAt, readiness.timezone)}</p></div><StatusBadge status={readiness.status} /></div><div style={metricsGridStyle}><Metric label="Buyer orders" value={readiness.aggregation.orderCount} /><Metric label="Awaiting full payment" value={readiness.aggregation.buyersAwaitingPayment} /><Metric label="Linked dispatches" value={readiness.linkedDispatches.length} /><Metric label="Next reservation expiry" value={readiness.aggregation.nextReservationExpiry ? formatDateTime(readiness.aggregation.nextReservationExpiry, readiness.timezone) : "None"} /></div><h3 style={subheadingStyle}>Compatible totals</h3>{readiness.aggregation.groupedTotals.map((group) => <div key={`${group.cropType}-${group.unit}`} style={summaryRowStyle}><strong>{group.cropType} · {group.unit}</strong><span>requested {group.requestedQuantity} · reserved {group.reservedQuantity} · paid {group.paidQuantity} · unpaid {group.unpaidQuantity}</span></div>)}<LoadMetric label="Minimum load" value={readiness.aggregation.minimumLoad} /><LoadMetric label="Capacity" value={readiness.aggregation.capacity} />{readiness.operationalConfirmationRequired && <p style={warningStyle}>Load readiness needs operational confirmation because configured capacity and order quantities do not share one compatible unit.</p>}<h3 style={subheadingStyle}>Blocking orders</h3>{readiness.blockers.length === 0 ? <p style={smallText}>No buyer payment blockers.</p> : readiness.blockers.map((blocker) => <div key={blocker.buyerOrderId} style={summaryRowStyle}><strong>{blocker.buyerName}</strong><span>{blocker.paymentStatus.replaceAll("_", " ")} · pay by {blocker.paymentDeadline ? formatDateTime(blocker.paymentDeadline, readiness.timezone) : "not set"}</span></div>)}<div style={{ ...actionsStyle, marginTop: 16 }}>{readiness.status === "draft" && <button style={primaryButton} onClick={() => void updateRunStatus({ actorUserId, runId: readiness._id, status: "accepting_orders" })}>Open orders</button>}{readiness.status === "cutoff_reached" && <button style={primaryButton} onClick={() => void updateRunStatus({ actorUserId, runId: readiness._id, status: "ready" })}>Mark ready</button>}{readiness.status === "ready" && <button style={primaryButton} onClick={() => void updateRunStatus({ actorUserId, runId: readiness._id, status: "confirmed" })}>Confirm run</button>}</div>{!["cancelled", "dispatched", "completed"].includes(readiness.status) && <div style={{ ...formGridStyle, marginTop: 18 }}><Label text="Cancellation or postponement reason"><input value={reason} onChange={(e) => setReason(e.target.value)} style={inputStyle} /></Label><Label text="New date for postponement"><input type="date" value={postponedDate} onChange={(e) => setPostponedDate(e.target.value)} style={inputStyle} /></Label><div style={actionsStyle}><button style={dangerButton} onClick={() => void updateRunStatus({ actorUserId, runId: readiness._id, status: "cancelled", reason }).catch((error: unknown) => setMessage(error instanceof Error ? error.message : "Run could not be cancelled."))}>Cancel run</button><button style={secondaryButton} onClick={() => void postponeRun({ actorUserId, runId: readiness._id, newDeliveryDate: postponedDate, reason }).then(setSelectedRunId).catch((error: unknown) => setMessage(error instanceof Error ? error.message : "Run could not be postponed."))}>Postpone to new date</button></div></div>}</section>}
    </div>
  );
}
