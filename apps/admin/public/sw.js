...
async function navigation(request) { const url = new URL(request.url); if (url.pathname.startsWith("/auth") || url.searchParams.has("_rsc")) return fetch(request); const cache = await caches.open(PAGE_CACHE); try { const response = await fetch(request); if (response.ok && url.pathname === "/") void cache.put(request, response.clone()); return response; } catch { return (await cache.match(request)) || (await caches.match(OFFLINE_URL)); } }
