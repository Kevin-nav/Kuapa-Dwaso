...
export default function FarmerOffersPage() {
  ...
  return (
    <div className="pilot-farmer-stack">
          ...
          programmes={[
            {
              ...
              ...(programme.demoContext.datasetId === undefined
                ? {}
                : { datasetId: programme.demoContext.datasetId }),
            },
          ]}
        />
      ) : null}
      <header className="pilot-farmer-heading">
        <span className="pilot-buyer-kicker">Your maize</span>
        <h1>Offers and supply</h1>
        <p>
          Next actions come first. An offer is not accepted until the server
          confirms your decision.
        </p>
        <Link className="btn btn-primary" href="/farmer/supply">
          <PlusCircle size={18} /> Declare maize supply
        </Link>
      </header>
      <section className="pilot-buyer-section">
        <div className="section-title-row">
          <h2 className="section-title">Offer inbox</h2>
          <span className="pilot-buyer-count">{offers.length}</span>
        </div>
        {programme !== undefined && supply === undefined ? (
          <div className="skeleton" style={{ height: 140, borderRadius: 12 }} />
        ) : null}
        {supply !== undefined && offers.length === 0 ? (
          <div className="pilot-buyer-empty">
            <Inbox size={30} />
            <h3>No offer yet</h3>
            <p>
              Your declarations stay under your control. We will show the full
              terms here when supply is matched.
            </p>
          </div>
        ) : null}
        <div className="pilot-request-list">
          {offers.map((offer) => (
            <Link
              ...
              href={`/farmer/offers/${offer.offerId}`}
              key={offer.offerId}
            >
              <div className="pilot-request-row-top">
                <strong>{offer.declaration.maizeType}</strong>
                <PilotStatus
                  label={offer.status}
                  tone={
              ...
              <div className="pilot-request-metrics">
                <span>
                  <b>{formatPilotQuantity(offer.terms?.offeredGrams ?? 0)}</b>{" "}
                  offered
                </span>
                <span>
                  <b>
                    {formatPilotMoney(
                      offer.finalAmounts?.expectedNetPesewas ??
                        offer.terms?.expectedNetPesewas ??
                        0,
                    )}
                  </b>{" "}
                  {offer.finalAmounts === null ? "expected net" : "final net"}
                </span>
              </div>
      ...
      <section className="pilot-buyer-section">
        <div className="section-title-row">
          <h2 className="section-title">Supply declarations</h2>
        </div>
        <div className="pilot-request-list">
          {supply?.page.map(({ declaration }) => (
            <article
              className="pilot-request-row"
              key={declaration.declarationId}
            >
              <div className="pilot-request-row-top">
                <strong>{declaration.maizeType}</strong>
                <PilotStatus label={declaration.status} tone="info" />
              </div>
              <div className="pilot-request-metrics">
                <span>
                  <b>{formatPilotQuantity(declaration.availableGrams)}</b>{" "}
                  declared
                </span>
                <span>
                  <b>{formatPilotQuantity(declaration.unallocatedGrams)}</b> not
                  committed
                </span>
              </div>
            </article>
