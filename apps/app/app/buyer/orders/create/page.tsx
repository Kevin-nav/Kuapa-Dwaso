...
function CreateOrderContent() {
  ...
  return <div style={{ display: "grid", gap: 18 }}>
    ...
    <header><p className="eyebrow">Scheduled delivery</p><h1>Place an order</h1><p>Kuapa Dwaso will prepare the selected stock for delivery. Pay before the deadline shown below.</p></header>
    ...
    {(runs ?? []).length > 0 && <form onSubmit={(event) => void submit(event)} className="auth-card" style={{ width: "100%", gap: 16 }}>
      ...
      <div className="field-stack"><label htmlFor="produce">Available produce</label><select id="produce" className="form-select" value={effectiveOffering} onChange={(event) => setOffering(event.target.value)} required><option value="">Choose produce</option>{offerings.map(([key, item]) => <option key={key} value={key}>{item.cropType} · grade {item.grade} · {item.unit}</option>)}</select><span className="field-help">{inventory === undefined ? "Checking reservable stock…" : `${availableQuantity.toLocaleString()} ${unit || "units"} currently reservable for this run.`}</span></div>
