...
export default function NewBuyerRequestPage() {
  ...
  const programmes = useQuery(api.pilotProgrammes.listAvailable, { limit: 20 }) as
    | { page: Array<{ id: Id<"pilotProgrammes">; name: string; status: string; datasetProvenance: "live" | "sample_only" }> }
    | undefined;
  ...
  useEffect(() => {
    const timeoutId = window.setTimeout(() => {
      ...
      try {
        const parsed = JSON.parse(saved) as { value?: Draft; expiresAt?: number };
        if (parsed.value !== undefined && (parsed.expiresAt ?? 0) > Date.now()) {
  ...
  useEffect(() => {
    window.localStorage.setItem(
      DRAFT_KEY,
      JSON.stringify({ value: draft, expiresAt: Date.now() + 7 * 24 * 60 * 60 * 1000 }),
    );
  }, [draft]);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!navigator.onLine) {
      setError("You are offline. Your draft is safe on this device; reconnect before submitting commercial terms.");
      return;
    }
    ...
    const moisturePercent = Number(draft.moisturePercent);
    const deliveryStart = new Date(`${draft.deliveryDate}T08:00:00+00:00`).getTime();
    if (!Number.isFinite(kilograms) || kilograms <= 0 || !Number.isFinite(deliveryStart)) {
    ...
    try {
      const creationIdempotencyKey = draft.creationIdempotencyKey ?? crypto.randomUUID();
      const created = draft.serverRequestId === undefined || draft.serverVersion === undefined
        ? await createDraft({
            programmeId: programme.id,
            maizeType: draft.maizeType,
      ...
      window.localStorage.setItem(DRAFT_KEY, JSON.stringify({ value: recoverable, expiresAt: Date.now() + 7 * 24 * 60 * 60 * 1000 }));
      await submitRequest({
      ...
      setError(cause instanceof Error ? cause.message : "The request could not be submitted. Your draft remains saved.");
    } finally {
  ...
  return (
    <div className="pilot-buyer-stack">
      ...
      <form className="pilot-request-form" onSubmit={(event) => void handleSubmit(event)}>
        <label className="form-group"><span className="form-label">Maize type</span><input className="form-input" required value={draft.maizeType} onChange={(event) => changeDraft({ maizeType: event.target.value })} /></label>
        <label className="form-group"><span className="form-label">Quantity (kg)</span><input className="form-input" inputMode="numeric" min="1" required type="number" value={draft.kilograms} onChange={(event) => changeDraft({ kilograms: event.target.value })} /><small>We track exact kilograms through sourcing, inspection, and delivery.</small></label>
        <label className="form-group"><span className="form-label">Delivery destination</span><input className="form-input" placeholder="e.g. Accra processing site" required value={draft.destination} onChange={(event) => changeDraft({ destination: event.target.value })} /></label>
