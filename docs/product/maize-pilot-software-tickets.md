...
| KD-15 | Complete | Added a distinct admin maize-pilot control room with sample/live programme selection, risk-first request and issue oversight, versioned commercial policy drafting/approval, guarded activation, sample programme creation, invitation handoff, and explicit zero-warehouse operator grants/revocation. Added finance-only purchasing budget evidence, accepted-offer approval queue, per-request immutable statements, evidence-backed partial/full settlement, actual coordination-cost posting, and aggregate reporting that separates gross produce value, fees, liabilities, costs, cash and incomplete contribution. Live purchase funding now requires approved commercial configuration and respects the configured per-reservation limit; sample provenance remains visible and programme-scoped rather than entering legacy warehouse reports. Analysts and operations roles retain read-only finance access because both the UI and mutations require `pilotFinance:manage`. Verified all 14 workspace test tasks (111 API, 71 utility, 22 permission and 11 validator tests), all 13 typecheck tasks, targeted admin/dashboard lint and strict direct Convex compilation; full lint contains only the documented pre-existing warnings outside these files. |
| KD-16 | Complete | Reframed the public journey around a controlled commercial-maize software pilot while retaining the demand-before-movement headline. Primary buyer and farmer CTAs now carry only allowlisted `request_maize_supply` or `sell_maize` intent through signup, preselect the explanatory path, and land a legitimately created or existing profile on its maize action; arbitrary values and external return URLs are ignored and intent never grants a role or programme scope. Farmer onboarding no longer requires an active or invented warehouse, while explicit legacy warehouse selection remains validated. Added clear farmer/buyer explanations, conditional warehouse progression, honest pilot timing, corrected footer anchors, and lighter role-specific login copy. Both story seeds now contain dated corrections to present-tense warehouse claims; the Extech visit is explicitly a historical learning visit with prospective, transaction-specific collaboration rather than an order or endorsement. Added exact-ID/timestamp fail-closed dry-run migration instructions and did not publish live content. Verified 74 utility tests including intent/open-redirect cases, app/www/utils typechecks and lint (one pre-existing buyer inventory image warning), strict direct compilation of changed Convex modules, and public imports remain free of dashboard/admin packages. |
| KD-17 | Not started | |
| KD-18 | Not started | |
| KD-19 | Not started | |
| KD-20 | Not started | |
